...
public class Searcher {
     ...
     * @param campo campo a realiza a pesquisa
     * @param out apenas o arquivo onde será imprimido o resultado de teste
    ...
    public void pesquisar(String valor, String campo, BufferedWriter out)
        ...
        if (!valor.trim().equals("")) {
                    ...
                    switch (i) {
                        ...
                        case 1:
                            ...
                            bq.add(new FuzzyQuery(new Term(campo, tokens[i])),
                                    Occur.SHOULD);
                            bq.add(new LengthQuery(campo, valor), Occur.MUST_NOT);
                            break;
                        default:
                            break;
                    }
            ...
            if (acronymBq != null) {
                ...
                //new QueryParser(Version.LUCENE_47, campo, new StandardAnalyzer(Version.LUCENE_47)).parse(query)
                //wrapBq.add(acronymBq, Occur.MUST_NOT);
            }
            // Pegando os documentos encontrado na pesquisa
            ScoreDoc[] hits = searcher.search(wrapBq, 100).scoreDocs;
            if (hits.length >= 0) {
